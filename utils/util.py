...
def anisotropic_gaussian_kernel(batch, kernel_size, covar):
    ...
    #kernel = kernel + torch.from_numpy(np.random.uniform(0, 0.25, (batch, kernel_size, kernel_size))).float().cuda() * kernel
...
def random_anisotropic_gaussian_kernel(batch=1, kernel_size=21, lambda_min=0.2, lambda_max=4.0):
    theta = (torch.rand(batch) * 2 - 1).cuda() * math.pi
    ...
    def __call__(self, hr_tensor, random=True):
        with torch.no_grad():
            ...
            if self.mode == 'bicubic':
                ...
                lr_blured = hr_blured.view(-1, C, H//int(self.scale), int(self.scale), W//int(self.scale), int(self.scale))[:, :, :, 0, :, 0]

            # add noise
            if self.noise > 0:
                _, C, H_lr, W_lr = lr_blured.size()
                noise_level = torch.rand(B, 1, 1, 1, 1).to(lr_blured.device) * (self.noise / (255 / self.rgb_range)) if random else (self.noise / (255 / self.rgb_range))
                noise = torch.randn_like(lr_blured).view(-1, N, C, H_lr, W_lr).mul_(noise_level).view(-1, C, H_lr, W_lr)
                lr_blured.add_(noise)

            lr_blured = torch.clamp(lr_blured, 0, self.rgb_range)
