...
class checkpoint():
    ...
    def save_results(self, filename, save_list, scale):
        ...
        normalized = save_list[0][0].data.mul(255 / self.args.rgb_range).clamp(0, 255).round()
...
def calc_psnr(sr, hr, scale, rgb_range, benchmark=False):
    ...
    if benchmark:
        ...
        if diff.size(1) > 1:
            ...
            '''
            convert[0, 0, 0, 0] = 65.481
            convert[0, 1, 0, 0] = 128.553
            convert[0, 2, 0, 0] = 24.966
...
def calc_ssim(img1, img2, scale=2, benchmark=False):
    ...
    '''
    img1_y = np.dot(img1, [65.481, 128.553, 24.966]) / 255.0 + 16.0
    img2_y = np.dot(img2, [65.481, 128.553, 24.966]) / 255.0 + 16.0
